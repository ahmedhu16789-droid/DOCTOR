      ...
      return (
          <div className="fixed inset-0 bg-gray-900 text-white z-50 flex flex-col p-8">
              <div className="flex justify-between items-center mb-8 border-b border-gray-700 pb-4">
                  ...
                  <button onClick={() => setTvMode(false)} className="px-4 py-2 bg-gray-800 rounded text-sm hover:bg-gray-700">Exit TV Mode</button>
              </div>
              <div className="flex-1 grid grid-cols-2 gap-8">
                  {/* Currently Called */}
                  <div className="bg-primary-600 rounded-3xl p-8 flex flex-col items-center justify-center animate-pulse">
                      <h2 className="text-3xl font-medium mb-4 uppercase tracking-widest opacity-80">Now Calling</h2>
                      {called.length > 0 ? (
                          <>
                            ...
                            <div className="mt-4 text-xl">Room 3</div>
                          </>
                      ) : (
                          <div className="text-3xl opacity-50">Please Wait...</div>
                      )}
                  </div>

                  {/* Up Next List */}
                  <div className="bg-gray-800 rounded-3xl p-8 overflow-hidden">
                       <h2 className="text-2xl font-bold mb-6 text-gray-400 uppercase">Up Next</h2>
                       <div className="space-y-4">
                           ...
                           {waiting.length === 0 && <p className="text-gray-500 italic">No patients waiting.</p>}
                       </div>
                  </div>
              </div>
              <div className="mt-8 text-center text-gray-500 text-sm">
                  Al-Fath Clinic • {new Date().toLocaleDateString()}
              </div>
          </div>
      );
  }
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-gray-200">
          ...
          <div className="flex gap-2">
              <div className="flex items-center gap-2 mr-4 text-sm text-gray-500">
                 <span className="flex items-center"><span className="w-2 h-2 rounded-full bg-amber-500 mr-1"></span> Waiting: {waiting.length}</span>
                 <span className="flex items-center"><span className="w-2 h-2 rounded-full bg-primary-500 mr-1"></span> In-Progress: {inProgress.length}</span>
              </div>
              <button 
              ...
              >
                  <Monitor className="w-4 h-4" /> TV Mode
              </button>
          </div>
      </div>
              ...
              <tr key={apt.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                   <div className="flex justify-end gap-2">
                       {/* Payment Trigger */}
                       <button onClick={() => setPaymentApt(apt)} className="text-gray-500 hover:text-green-600 p-1 rounded hover:bg-green-50" title="Process Payment">
                       ...
                       {apt.status === AppointmentStatus.SCHEDULED && (
                           <>
                            <button onClick={() => handleAction(apt.id, apt.status, 'checkin')} className="text-primary-600 hover:text-primary-900 bg-primary-50 px-3 py-1 rounded">Check In</button>
                            <button onClick={() => handleAction(apt.id, apt.status, 'noshow')} className="text-red-600 hover:text-red-900 px-3 py-1">No Show</button>
                           </>
                       )}
                       ...
                       {apt.status === AppointmentStatus.WAITING && (
                           <button onClick={() => handleAction(apt.id, apt.status, 'call')} className="flex items-center gap-1 text-amber-600 hover:text-amber-900 bg-amber-50 px-3 py-1 rounded">
                               <Megaphone className="w-3 h-3" /> Call
                           </button>
                       )}
                       ...
                       {apt.status === AppointmentStatus.COMPLETED && (
                           <span className="text-gray-400 flex items-center justify-end gap-1"><CheckCircle className="w-3 h-3" /> Done</span>
                       )}
                   </div>
                </td>
              </tr>
