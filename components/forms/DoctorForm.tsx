...
export const DoctorForm: React.FC<DoctorFormProps> = ({ initialData, branches, departments, onSave, onCancel, isSaving }) => {
  ...
  const onSubmit = async (data: DoctorFormValues) => {
    const userPayload: User = {
      ...
      services: (data.services || []).filter((service) => service.name.trim() !== '').map((service) => ({ ...service, name: service.name.trim() })),
      ...
      <div className="flex-1 overflow-y-auto p-6 bg-gray-50">
        {allErrorMessages.length > 0 && (
          <div className="mb-4 rounded border border-red-200 bg-red-50 p-3 text-sm text-red-700">
            {allErrorMessages.map((msg, idx) => (<div key={idx}>{msg as string}</div>))}
          </div>
        )}
              ...
              {(paymentModel === 'PERCENTAGE' || paymentModel === 'HYBRID') && (
                <div className="md:col-span-2 space-y-3 border border-gray-200 rounded-lg p-4">
                  <label className="flex items-center gap-2 text-sm font-medium text-gray-700">
                    <input {...register('payroll.additionalServicesCommissionEnabled')} type="checkbox" className="h-4 w-4 rounded border-gray-300" />
                  ...
                  {watch('payroll.additionalServicesCommissionEnabled') && (
                    <div>
                      ...
                      <input {...register('payroll.additionalServicesCommissionPercentage', { valueAsNumber: true })} type="number" min={0} max={100} className="block w-full rounded-md border-gray-300 border p-2" />
                    </div>
                  )}
                </div>
              )}
