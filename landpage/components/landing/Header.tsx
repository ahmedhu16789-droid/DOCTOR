...
import type { HeaderData } from "@/types/landing";
...
export function Header({ brand, header }: HeaderProps) {
  return (
    <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/95 backdrop-blur">
      ...
      <Container className="flex h-16 items-center justify-between gap-4">
        <Link href={brand.href ?? "#"} className="text-lg font-bold text-slate-900">
          {brand.logoText}
        </Link>
