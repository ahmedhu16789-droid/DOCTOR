        ...
        <div key={doctor.id} className="bg-white border border-gray-200 rounded-lg p-4 mb-3 shadow-sm flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <div className="flex items-center w-full sm:w-64 flex-shrink-0">
                ...
                <div className="ml-3">
                    ...
                    <p className="text-xs text-gray-500 capitalize">{doctor.role.toLowerCase()}</p>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-x-auto w-full">
                <div className="flex gap-2 pb-2 sm:pb-0">
                    ...
                    {slots.filter(s => s.available).length === 0 && (
                        <span className="text-xs text-red-400 font-medium py-2">No slots available</span>
                    )}
                </div>
            </div>
        </div>
    ...
    <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden flex flex-col h-full max-h-[800px]">
      ...
      <div className="bg-gray-50 px-6 py-4 border-b border-gray-100 flex justify-between items-center">
        <h2 className="text-lg font-bold text-gray-900 flex items-center">
             ...
             {step === 'IDENTIFICATION' ? 'Identification' : step === 'SELECTION' ? 'Select Slot' : 'Confirm'}
        </h2>
        {step !== 'IDENTIFICATION' && (
             <button onClick={() => setStep('IDENTIFICATION')} className="text-sm text-gray-500 hover:text-red-600">Cancel</button>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
         ...
         {step === 'SELECTION' && (
             <div className="space-y-6">
                 ...
                 <div className="flex flex-col md:flex-row gap-4 bg-gray-50 p-4 rounded-lg border border-gray-200">
                    <div className="flex-1">
                        ...
                        <div className="font-bold text-gray-900 flex items-center">
                            ...
                            <button onClick={() => setStep('IDENTIFICATION')} className="ml-2 text-primary-600 text-xs hover:underline">(Change)</button>
                        </div>
                    </div>
                    <div className="flex-1">
                 ...
                 {/* Availability Grid */}
                 <div>
                     <h3 className="text-sm font-bold text-gray-500 mb-3 uppercase tracking-wider">Available Doctors & Slots</h3>
                     {doctors.length === 0 ? (
         ...
         {step === 'CONFIRMATION' && (
             <div className="max-w-md mx-auto">
                 <div className="bg-green-50 border border-green-200 rounded-xl p-6 text-center mb-6">
                    <h3 className="text-xl font-bold text-green-800 mb-1">Confirm Booking</h3>
                    <p className="text-green-600 text-sm">Please verify details below</p>
                 </div>
                 ...
                 <div className="flex gap-4">
                     <button onClick={() => setStep('SELECTION')} className="flex-1 py-3 border border-gray-300 rounded-lg font-medium hover:bg-gray-50">Back</button>
                     <button onClick={handleConfirm} className="flex-[2] py-3 bg-primary-600 text-white rounded-lg font-bold hover:bg-primary-700 shadow-lg">Confirm Booking</button>
                 </div>
             </div>
         )}
      </div>
